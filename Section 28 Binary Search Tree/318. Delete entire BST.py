...
def deleteNode(root, delNode): # time complex O(logn) space complex O(logn)
    if not root:
        return root
    
    if delNode < root.data:
        root.leftChild = deleteNode(root.leftChild, delNode)
    elif delNode > root.data:
        root.rightChild = deleteNode(root.rightChild, delNode)
    else:
...
def deleteBST(root): # time complex O(1) space complex O(1)
    root.data = None
    root.leftChild = None
