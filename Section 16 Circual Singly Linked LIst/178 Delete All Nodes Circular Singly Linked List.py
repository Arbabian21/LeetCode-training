...
class CLinkedList:
    ...
    def remove(self, index): # time complex O(n)
        if self.length < index < 0 or self.head == None:
            return None
    ...
    def deleteAll(self): # time complex O(1)
        if self.head == None:
            return
